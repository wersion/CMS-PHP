...
// class Image_inc
// {
//   private $file_path;//图片路径
//   private $file_width;
//   private $file_height;
//   private $file_type;//图片类型
//   private $img;//图片资源句柄
//   private $new;//新图的资源句柄
...
//     // 获取图片的宽、高、类型
//     list($this->file_width,$this->file_height,$this->file_type) = getimagesize($this->file_path);
//     // 保存图片的资源句柄
//     $this->img = $this->GetImageHandle($this->file_path,$this->file_type);
//   }
...
//       $new_width = ($new_height / $this->file_height) * $this->file_width;
//     } else {
//       $new_height = ($new_width / $this->file_width) * $this->file_height;
//     }
//     if ($new_width < $_n_w) { //如果新高度小于新容器高度
//       $r = $_n_w / $new_width; //按长度求出等比例因子
//       $new_width *= $r; //扩展填充后的长度
//       $new_height *= $r; //扩展填充后的高度
//       $_cut_height = ($new_width - $_n_h) / 2; //求出裁剪点的高度
//     }
//     if ($new_height < $_n_h) { //如果新高度小于容器高度
//       $r = $_n_h / $new_height; //按高度求出等比例因子
//       $new_width *= $r; //扩展填充后的长度
//       $new_height *= $r; //扩展填充后的高度
//       $_cut_width = ($new_height - $_n_w) / 2; //求出裁剪点的长度
//     }
//     $this->new = imagecreatetruecolor($_n_w,$_n_h);
//     imagecopyresampled($this->new,$this->img,0,0,$_cut_width,$_cut_height,$new_width,$new_height,$this->file_width,$this->file_height);
//   }

//   // 获取图片的资源句柄
//   private function GetImageHandle($_file,$_type){
//     switch ($_type) {
//       case 1 :
...
//   //图像输出
//   public function out() {
//     imagepng($this->new,$this->file_path);
