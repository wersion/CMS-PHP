...
<html lang="zh-CN">
  ...
  <div class="container min-height wite">
    {if isset($defaultPage)&&$defaultPage == true}
      <form method="post">
      ...
      {elseif isset($setPageTemplate)&&$setPageTemplate==true}
      <form method="post">
        <table class="table table-hover">
          <tr><th>ID</th><th>标题</th><th>模板</th><th><input type="checkbox"/>本页全选</th></tr>
          {foreach from=$page key=key item=item}
            <tr>
              <td>{$item.pageID}</td>
              <td>{$item.pageName}</td>
              <td>{$item.templateName}</td>
              <td><input type="checkbox" name="checkbox[]" value="{$item.pageID}" /></label></td>
            </tr>
          {/foreach}
      </table>
      <div class="page">
